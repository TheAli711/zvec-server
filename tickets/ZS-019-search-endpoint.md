...
- [x] `POST /collections/{name}/search` returns 200 with `{"results": [...]}`, each hit a
      `DocOut` with `id`, `score`, `vectors` (only when `include_vector` is true), and
      `fields`.
- [x] `output_fields` restricts the scalar fields in each hit; `null` returns all.
- [x] Several queries in one request run through a single engine call; search by `id`
      returns the referenced document among the hits.
- [x] The handler resolves the collection with `manager.get` (404 missing, 503
      unavailable) and runs via `managed.read`, never blocking the event loop.
- [x] A filter using `==` returns 400 `invalid_argument`; a query with neither `vector`
      nor `id` returns 422.
- [x] An integration test restarts the app on the same data directory and gets the same
      hit back.
...
Added the search route to `api/vectors.py` and `SearchResponse` to `models/search.py`.
`tests/integration/test_search_api.py` covers vector and id queries, filtered search with
`output_fields`, the 400/404/422 paths, and a persistence reload across two app
instances. The response stayed a single flat list; per-query result lists were not
added.
