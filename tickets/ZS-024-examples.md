...
- [ ] `examples/python_client.py` and `examples/curl_examples.sh` run health, create
      `articles_example`, insert, filtered search, fetch `a1` with its vector, update,
      delete `a3`, and drop the collection.
- [ ] Both read `ZVEC_SERVER_URL` (default `http://localhost:8000`) and send
      `Authorization: Bearer $ZVEC_SERVER_API_KEY` when that variable is set.
- [ ] The Python client drops the collection in a `finally` block and raises with the
      server's error body on any non-2xx response.
- [ ] The curl script uses `set -euo pipefail`, pretty-prints with `jq` when available,
      and fails with the response body shown on HTTP status 400 or above.
- [ ] `examples/README.md` explains prerequisites and how to run each example.
