...
- [x] `runtime.init_zvec(log_level, log_dir, memory_limit_mb, query_threads,
...
- [x] Log levels map from Python names (`WARNING` → `WARN`, `CRITICAL` → `FATAL`);
      unknown values fall back to `WARN`.
- [x] `enums.py` exposes `VECTOR_DATA_TYPES`, `SCALAR_DATA_TYPES`, `INDEX_TYPES`,
...
- [x] `schema_mapper.build_collection_schema(name, vectors, fields)` builds HNSW
...
- [x] A vector spec with a scalar dtype (or the reverse) is rejected;
      `primary_vector_info(req)` returns the first vector's `(name, dim)`.
- [x] Unit tests in `test_enums.py` and `test_schema_mapper.py` run against the
      real `zvec` module.
...
Added `adapter/__init__.py`, `runtime.py`, `enums.py`, and `schema_mapper.py` with
unit tests. The lifespan calls `init_zvec` with the engine settings from SPEC-001.
