...
- [ ] `pyproject.toml` declares `zvec>=0.7.0`, and `uv.lock` resolves `zvec` 0.7.0.
- [ ] `ruff check`, `ruff format --check`, `mypy`, and `pytest` pass on Python 3.12
      and 3.13 with no adapter changes, or any needed changes stay inside
      `zvec_server.adapter`.
- [ ] With `--mmap`, a benchmark run returns no empty ids, and recall matches
      `--no-mmap` (ZS-040).
- [ ] The mmap caveat in `benchmarks/README.md` is updated to match.
