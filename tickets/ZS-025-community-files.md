...
- [x] `CONTRIBUTING.md` covers scope, setup (`uv sync --extra dev`, optional pre-commit),
      the four quality gates, coding standards and layering, the docs-in-the-same-PR
      rule, the PR process, and bug reporting.
- [x] `SECURITY.md` states the threat model (trusted network, optional single key, no
...
- [x] `CODE_OF_CONDUCT.md` adopts Contributor Covenant 2.1 with an enforcement contact.
- [x] `.github/ISSUE_TEMPLATE/bug_report.md` asks for the exact request/response and
      environment; `feature_request.md` includes a scope check.
- [x] `.github/PULL_REQUEST_TEMPLATE.md` has a change-type list and a checklist for
      ruff, format, mypy, pytest, docs, CHANGELOG, and scope.
- [x] `CHANGELOG.md` follows Keep a Changelog and SemVer, with `[Unreleased]` and a
...
Added `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, both issue templates, the
PR template, and `CHANGELOG.md` with the 0.1.0 entry. One inconsistency shipped: the
feature-request scope check still lists authentication as out of scope, while
CONTRIBUTING (correctly) lists only authorization.
