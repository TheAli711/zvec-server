...
- [ ] An integration test creates a quantized `hnsw` collection (`int4`,
      `enable_rotate: true`) and a SOAR `ivf` collection (`n_list: 4`,
      `use_soar: true`), seeds 300 documents in each, and optimizes both.
- [ ] After restarting the app on the same data directory, both collections report
      `available: true`, identical `vectors` (including `index_param`), and
      `doc_count` 300.
- [ ] The quantized collection still reports `INT4` with rotation, and the IVF one
      reports `use_soar: true`.
- [ ] After the restart, search by id with `{"ef": 64}` on the hnsw field and
