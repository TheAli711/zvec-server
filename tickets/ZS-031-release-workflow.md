...
- [ ] The workflow triggers on `release: published`. Both jobs are guarded by
      `startsWith(github.event.release.tag_name, 'v')`.
- [ ] The `test` job runs `uv sync --extra dev` and `uv run pytest` on Python 3.12
      and 3.13 with `fail-fast: false`. `publish` has `needs: test`.
- [ ] `publish` logs in to `ghcr.io` with `GITHUB_TOKEN` and is the only job granted
      `packages: write`; the workflow default is `contents: read`. It builds with
      Buildx and the GitHub Actions cache.
- [ ] Tags come from `docker/metadata-action`: `v{{version}}`,
      `v{{major}}.{{minor}}`, and `latest=auto`. A pre-release gets only its full
      version tag.
- [ ] The build sets `provenance: false`, so the pushed image is a plain
