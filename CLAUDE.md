...
- **Concurrency.** Each `ManagedCollection` owns a fair reader/writer lock. Reads
  ...
  marked unavailable first. Blocking Zvec calls run in
...
Work flows **spec → tickets → tracker**, all in-repo (process: `specs/README.md`):

- `specs/SPEC-NNN-*.md` — design doc written before code; `status:` draft →
  accepted → implemented.
- `tickets/ZS-NNN-*.md` — units of work, each tied to a spec (`spec:`); bugs are
  tickets too. `status:` backlog / todo / in-progress / done / wontfix.
- `TRACKER.md` — **generated** board; never hand-edit. Regenerate with
  `python scripts/tracker.py`; CI runs `--check` (a stale board or bad front
  matter fails the build).

For any behavior change: make sure a ticket exists (file one, plus a spec for a
new capability); in the same commit that lands the change, flip it to `done`
with a `closed:` date, tick its criteria, add a `## Resolution`, regenerate the
tracker, and put a `Closes: ZS-NNN` trailer in the commit message.
